...
export function IconChip({
  ...
  icon: LucideIcon;
  tone?: 'accent' | 'muted';
  /** Color explícito (familia de mascotas de FICHA-ARTE) — para variedad de color
      entre chips de una misma lista, sin salirse de la paleta aprobada. */
  color?: string;
}) {
  const acento = tone === 'accent';
  const c = color ?? (acento ? 'var(--accent)' : 'var(--text-secondary)');
  return (
    <span
      aria-hidden="true"
      className="inline-flex size-11 shrink-0 items-center justify-center rounded-[var(--radius-button)] border"
      style={{
        borderColor: `color-mix(in oklab, ${c} ${acento || color ? 22 : 35}%, transparent)`,
        background: `color-mix(in oklab, ${c} ${acento || color ? 12 : 12}%, transparent)`,
      }}
    >
      <Icono size={22} strokeWidth={2} color={c} aria-hidden="true" />
    </span>
  );
}
...
/* ── <CheckCustom> — círculo acento 12% + check SVG (55 repertorio #9).
   Nunca el símbolo de check del sistema ni un emoji. ── */
...
export function SectionShell({
  ...
  /* --surface y --bg son dos cremas muy próximos en valor (hallazgo del revisor:
     "la alternancia se percibe débil en el scroll largo") — una línea sutil marca
     la costura entre secciones en vez de depender solo del cambio de color. Se
     omite cuando la sección va "flush" contra la anterior (mismo movimiento visual). */
  const costura = elevacion === 'elevada' && flush !== 'top' ? 'border-t border-[color-mix(in_oklab,var(--text-primary)_7%,transparent)]' : '';
  return (
    <section
      id={id}
      aria-label={ariaLabel}
      className={`${elevacion === 'elevada' ? 'bg-[var(--surface)]' : ''} ${costura} ${pt} ${pb} ${className}`}
    >
...
export function CtaButton({
  ...
  // Tratamiento "flotante y glossy" (pedido explícito del usuario): mismo lenguaje
  // que los botones de la app (borde grueso + brillo diagonal + halo de color) —
  // desviación deliberada del CTA plano del kit, documentada en ESTADO.md.
  return (
    <motion.a
      whileTap={{ scale: 0.97 }}
      href={href}
      className={`hablapronto-cta relative inline-flex items-center justify-center overflow-hidden rounded-[var(--radius-button)] border-[2.5px] px-8 text-[17px] font-bold text-[var(--bg)] [touch-action:manipulation] ${
        alto === 56 ? 'h-14' : 'h-[52px]'
      } ${fullMobile ? 'w-full sm:w-auto' : ''}`}
    >
      <span aria-hidden="true" className="hablapronto-cta-sheen pointer-events-none absolute -left-[10%] -top-[45%] h-[70%] w-[55%] rounded-full" />
      ...
      {visible && (
        ...
        >
          <motion.a
            ...
            className="hablapronto-cta flex h-12 w-full items-center justify-center overflow-hidden rounded-[var(--radius-button)] border-[2px] text-[16px] font-bold text-[var(--bg)] [touch-action:manipulation]"
          >
