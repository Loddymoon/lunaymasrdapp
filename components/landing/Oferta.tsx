...
import { motion, animate, useReducedMotion, useInView } from 'motion/react';
...
function TrialBadge({ dias }: { dias: number }) {
  return (
    <span className="inline-flex w-fit items-center gap-1.5 rounded-full bg-[color-mix(in_oklab,var(--accent-2)_16%,transparent)] px-2.5 py-1 text-[11px] font-bold uppercase tracking-[0.06em] text-[color-mix(in_oklab,var(--accent-2)_75%,black)]">
...
/* El precio (dato más mirado de la página) cuenta de 0 al valor real cuando entra
   en el viewport — antes aparecía estático mientras el resto de la sección sí
   animaba (baseline de animación #2, 22/55 T4). Prefijo/sufijo no numéricos del
   string (ej. "$") se preservan tal cual, sin asumir formato de moneda fijo. */
function PrecioAnimado({ texto }: { texto: string }) {
  ...
  const valor = match ? parseFloat(match[2].replace(',', '.')) : 0;
  const [mostrado, setMostrado] = useState(reduceMotion ? valor : 0);

  useEffect(() => {
    // reduceMotion ya deja `mostrado` en su valor final desde el useState
    // inicial — no hace falta reafirmarlo aquí (evita setState síncrono
    // dentro del efecto, que dispara renders en cascada innecesarios).
    if (!match || reduceMotion || !enVista) return;
    const controls = animate(0, valor, { duration: 0.8, ease: 'easeOut', onUpdate: setMostrado });
  ...
  return (
    <SectionShell id={id} elevacion="base" ariaLabel="Planes y precios">
      <motion.div variants={contenedor} initial="hidden" whileInView="visible" viewport={VIEWPORT_ONCE}>
        ...
        {/* El badge de trial se muestra UNA vez para ambos planes (antes se repetía
            en cada tarjeta y competía con "MÁS POPULAR" — hallazgo del revisor). */}
        {trialDias !== undefined && (
        ...
        <div className="mx-auto mt-4 grid max-w-[880px] grid-cols-1 items-start gap-6 md:grid-cols-2">
          ...
          <motion.div variants={item} className="relative md:-translate-y-2">
            {anual.badge && (
              <span className="absolute -top-3 left-1/2 z-10 -translate-x-1/2 rounded-full border border-[color-mix(in_oklab,var(--accent)_25%,transparent)] bg-[var(--accent)] px-3 py-1 text-[11px] font-bold uppercase tracking-[0.06em] text-[var(--bg)]">
                {anual.badge}
              </span>
            )}
          ...
          >
            ...
            <motion.a
              ...
              className="mt-6 flex h-12 w-full items-center justify-center rounded-[var(--radius-button)] border-2 border-[color-mix(in_oklab,var(--accent-2)_55%,transparent)] text-[16px] font-bold text-[var(--accent-2)] transition-colors duration-150 hover:bg-[color-mix(in_oklab,var(--accent-2)_10%,transparent)] [touch-action:manipulation]"
            >
              {mensual.ctaLabel}
            </motion.a>
          </motion.div>
