...
function analizarTsx(file, src) {
  ...
  while ((lm = litRe.exec(src)) !== null) {
    ...
    // className={`...`} (template literal tras llave JSX) — el lookback original solo
    // cazaba className="..."/className='...', no className={`...`} con la llave de por medio.
    if (/\b(import|from|require|href=|src=|url\(|class(Name)?\s*=)\s*\{?\s*$/.test(ctx)) continue;
    if (/\b(import|from)\b/.test(ctx)) continue;
    // Heurística "parece código, no prosa": comentarios/JSX/Tailwind capturados por el regex
    // de comillas (una comilla dentro de un comentario puede abrir un match gigante hasta la
    // siguiente comilla real). Señales fuertes de código: sintaxis JS/JSX, utilidades Tailwind
    // con corchete, o muy baja proporción de letras/espacios frente a símbolos.
    if (/className=|=>|\bconst\s|\bfunction\s|\breturn\s*\(|\buseEffect\b|\buseState\b|\/\*\*|\*\/|\{\{|\}\}/.test(val)) continue;
    if (/\b[a-z-]+-\[/.test(val)) continue; // ej. rounded-[20px], text-[17px], w-[52px]
    const letras = (val.match(/[\p{L}\s]/gu) || []).length;
    if (letras / val.length < 0.75) continue; // prosa real es mayormente letras+espacios
...
function check10() {
  ...
  if (registro === 'tuteo' || registro === 'usted') {
    ...
    for (const a of analisis) {
      const esCodigo = /\.(tsx?|jsx?)$/.test(a.file);
      a.src.split('\n').forEach((ln, i) => {
        voseoRe.lastIndex = 0;
        let m;
        while ((m = voseoRe.exec(ln)) !== null) {
          // Falso positivo conocido: "animate" es prop de Motion/Framer (animate={{...}})
          // en TODO archivo .tsx/.jsx del SO, indistinguible por regex de la forma voseante
          // "animate" (de animarse). Si el match viene seguido de "=" en código, es la prop,
          // no prosa en español — se descarta (causa raíz del hallazgo #10, no supresión ciega:
          // solo aplica a este choque léxico específico, sigue detectando "animate" en prosa).
          const siguiente = ln.slice(voseoRe.lastIndex, voseoRe.lastIndex + 1);
          if (esCodigo && m[1].toLowerCase() === 'animate' && siguiente === '=') continue;
