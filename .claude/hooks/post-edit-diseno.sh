...
#    Alcance: SOLO utilidades de espaciado (padding/margin/gap/inset) — la regla 14 "Espaciado
#    mecánico" es sobre el ritmo entre bloques, no sobre tamaño de fuente (text-), radio
#    (rounded-), grosor de borde (border-) ni ancho de columna (max-w-/w-/h-), que siguen sus
#    propias escalas ya documentadas (tipografía 4 niveles y radios de FICHA-ARTE.md) — antes
#    este regex marcaba cualquier prefijo-[Npx], generando falsos positivos en valores ya
#    aprobados por el revisor-visual (ej. text-[15px] de cuerpo, rounded-[20px] de tarjetas).
ARB=$(grep -nE '\b(p|px|py|pt|pr|pb|pl|m|mx|my|mt|mr|mb|ml|gap|gap-x|gap-y|space-x|space-y|inset|inset-x|inset-y|top|right|bottom|left)-\[[0-9]+px\]' "$FILE_PATH" 2>/dev/null \
      | awk '{ rest = $0; bad = 0
